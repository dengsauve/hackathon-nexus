<x-layouts.app title="Create account - Hackathon Nexus">
    <x-auth-card title="Create account" subtitle="Set up a workspace login for the hackathon portal.">
        <div class="grid gap-3">
            <a href="{{ route('auth.social.redirect', 'google') }}" class="rounded-md border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-white/10">Sign up with Google</a>
            <a href="{{ route('auth.social.redirect', 'github') }}" class="rounded-md border border-white/15 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-white/10">Sign up with GitHub</a>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <label class="block">
                ...
                <input name="name" value="{{ old('name') }}" required autofocus class="mt-2 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-3 text-white outline-none ring-teal-300/50 focus:ring-2">
            </label>

            <label class="block">
                <span class="text-sm font-medium text-zinc-200">Email</span>
                <input name="email" type="email" value="{{ old('email') }}" required class="mt-2 w-full rounded-md border border-white/10 bg-zinc-950 px-3 py-3 text-white outline-none ring-teal-300/50 focus:ring-2">
            </label>
            ...
            <button class="w-full rounded-md bg-teal-300 px-4 py-3 font-semibold text-zinc-950 hover:bg-teal-200">Create account</button>
        </form>
